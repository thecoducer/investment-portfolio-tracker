...
class TestMarketDataClient(unittest.TestCase):
    ...
    def test_create_session_success(self, mock_session_class):
        ...
        mock_session.get.assert_called_once_with(
            self.client.base_url, headers=self.client.headers, timeout=self.client.timeout
        )
...
class TestZerodhaAPIClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.zerodha_client = ZerodhaAPIClient(
            self.mock_auth_manager, self.mock_holdings_service, self.mock_sip_service
        )
    ...
    def test_fetch_account_data_success(self):
        ...
        self.mock_holdings_service.fetch_holdings.return_value = ([{"stock": "data"}], [{"mf": "data"}])
    ...
    def test_fetch_all_accounts_data_success(self, mock_thread_class):
        ...
        mock_thread_class.side_effect = lambda target, args, daemon: Mock(
            start=Mock(side_effect=lambda: target(*args)), join=Mock()
        )

        # Set up mock returns
        self.mock_auth_manager.authenticate.return_value = Mock()
        self.mock_holdings_service.fetch_holdings.return_value = ([{"stock": "A"}], [{"mf": "A"}])
        ...
        self.mock_holdings_service.merge_holdings.return_value = ([{"stock": "merged"}], [{"mf": "merged"}])
        self.mock_sip_service.merge_items.return_value = [{"sip": "merged"}]

        accounts_config = [{"name": "Account1"}, {"name": "Account2"}]

        stocks, mfs, sips, error = self.zerodha_client.fetch_all_accounts_data(accounts_config)
