...
class TestNSEAPIClient(unittest.TestCase):
    ...
    def test_create_session_success(self, mock_session_class):
        ...
        session = self.nse_client._create_session()
        
        self.assertEqual(session, mock_session)
        mock_session.get.assert_called_once_with(
            self.nse_client.base_url,
            headers=self.nse_client.headers,
    ...
    def test_create_session_failure(self, mock_session_class):
        ...
        with self.assertRaises(Exception):
            self.nse_client._create_session()
    
    @patch.object(NSEAPIClient, '_create_session')
    def test_fetch_nifty50_symbols_success(self, mock_create_session):
        ...
        symbols = self.nse_client.fetch_nifty50_symbols()
    ...
    def test_fetch_stock_quote_success(self, mock_sleep):
        ...
        quote = self.nse_client.fetch_stock_quote(mock_session, 'RELIANCE')
    ...
    def test_fetch_stock_quote_http_error(self, mock_sleep):
        ...
        quote = self.nse_client.fetch_stock_quote(mock_session, 'INVALID')
    ...
    def test_fetch_stock_quote_exception(self, mock_sleep):
        ...
        quote = self.nse_client.fetch_stock_quote(mock_session, 'TCS')
    ...
    def test_empty_stock_data(self):
        ...
        empty_data = self.nse_client._empty_stock_data('TEST')
...
class TestZerodhaAPIClient(unittest.TestCase):
    ...
    def test_fetch_account_data_success(self):
        ...
        self.mock_auth_manager.authenticate.assert_called_once_with(account_config, False)
    
    def test_fetch_account_data_with_force_login(self):
        """Test account data fetch with force login"""
        mock_kite = Mock()
        self.mock_auth_manager.authenticate.return_value = mock_kite
        self.mock_holdings_service.fetch_holdings.return_value = ([], [])
        self.mock_sip_service.fetch_sips.return_value = []
        
        account_config = {'name': 'test_account'}
        self.zerodha_client.fetch_account_data(account_config, force_login=True)
        
        self.mock_auth_manager.authenticate.assert_called_once_with(account_config, True)
