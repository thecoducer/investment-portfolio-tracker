...
def enrich_holdings_with_prices(
    holdings: list[dict[str, Any]], gold_prices_data: dict[str, Any]
) -> list[dict[str, Any]]:
    ...
    for holding in holdings:
        ...
        if gold_prices:
            # Match purity string (numeric or karat notation) to IBJA price buckets:
            # 999/24K → 99.9% pure, 916/22K → 91.6%, 750/18K → 75%
...
def calculate_totals(holdings: list[dict[str, Any]]) -> dict[str, float]:
    ...
    total_invested = sum(h.get("weight_gms", 0) * h.get("bought_ibja_rate_per_gm", 0) for h in holdings)

    return {"total_weight_gms": total_weight, "total_invested": total_invested, "count": len(holdings)}
