...
def calculate_compound_interest(
    principal: float, annual_rate: float, time_in_years: float, compounding_frequency: int = 4
) -> float:
...
def calculate_current_value(fixed_deposits: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for deposit in fixed_deposits:
        ...
        current_value = calculate_compound_interest(principal, annual_rate, years_elapsed, compounding_frequency=4)
    ...
    enriched_deposits.sort(key=lambda d: parse_date(d.get("maturity_date")) or date.max)
