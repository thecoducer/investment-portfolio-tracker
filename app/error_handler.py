...
class PortfolioTrackerError(Exception):
    ...
    def __init__(
        self, message: str, category: ErrorCategory = ErrorCategory.UNKNOWN, original_error: Exception | None = None
    ):
...
class ErrorHandler:
    ...
    def wrap_external_api_error(error: Exception, service_name: str) -> PortfolioTrackerError:
        ...
        if isinstance(error, Timeout):
            return NetworkError(f"{service_name} request timeout - server slow to respond", original_error=error)
        elif isinstance(error, ConnectionError):
            return NetworkError(f"Cannot connect to {service_name} - network unavailable", original_error=error)
        elif isinstance(error, HTTPError):
            status_code = error.response.status_code if hasattr(error, "response") else None
            return APIError(
                f"{service_name} returned HTTP error: {error}", status_code=status_code, original_error=error
            )
        elif isinstance(error, RequestException):
            return NetworkError(f"{service_name} request failed: {error}", original_error=error)
        else:
            return PortfolioTrackerError(f"Unexpected error from {service_name}: {error}", original_error=error)
...
def handle_errors(
    default_return: Any = None, log_context: str = "", preserve_cache: bool = False, cache_attr: str | None = None
):
...
def safe_api_call(func: Callable, *args, **kwargs) -> tuple[Any | None, Exception | None]:
...
class ErrorAggregator:
    ...
    def add(self, error: Exception, context: str = ""):
        ...
        """
        self.errors.append({"error": error, "context": context, "message": str(error)})
    ...
    def get_summary(self) -> str:
        ...
        if len(self.errors) == 1:
            ...
            return (
                f"{error_info['context']}: {error_info['message']}" if error_info["context"] else error_info["message"]
            )
